...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<any> {
    ...
    if (user.isActive === false) {
      throw new ForbiddenException("Este usuário foi desativado.");
    }
  ...
  async login(user: any) {
    ...
    if (user.role === "operator") {
      console.log(
        `⚡ [Auth] Login de operador ${user.name} (ID: ${user.id}) - Iniciando alocação imediata de linha`,
      );
      ...
      setImmediate(async () => {
        try {
          ...
          if (!existingLine && !user.line) {
            ...
            console.log(
              `✅ [Auth] Alocação imediata processada para ${user.name}`,
            );
          } else {
            console.log(
              `ℹ️ [Auth] Operador ${user.name} já possui linha, pulando alocação`,
            );
          }
        } catch (error) {
          console.error(
            `❌ [Auth] Erro na alocação imediata pós-login: ${error.message}`,
          );
        }
      });
    }
