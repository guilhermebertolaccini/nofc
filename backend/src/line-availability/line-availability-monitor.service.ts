    ...
    try {
      ...
      for (const operator of operatorsWithLines) {
        ...
        if (evolution) {
          ...
          // Se linha está banida ou desconectada inicialmente na cache, forçar verificação real (verifyLineHealth)
          if (lineStatus && (lineStatus === 'ban' || lineStatus === 'disconnected' || lineStatus.toLowerCase() === 'ban' || lineStatus.toLowerCase() === 'disconnected')) {
            this.logger.warn(
              `Suspeita de linha ${line.phone} banida/desconectada (${lineStatus}) via cache. Acionando validação oficial (VerifyLineHealth)...`,
              'LineAvailability'
            );

            // Double-Check: Usar a mesmíssima lógica do botão "Verify" manual
            // Isso limpa o cache e faz requisição direta à porta da Evolution
            const verifyResult = await this.linesService.verifyLineHealth(line.id);
            const isConfirmedBanned = verifyResult.actionTaken === 'banned' || verifyResult.newStatus === 'ban';
            const realConnectionState = verifyResult.connectionState;

            // Se CONFIRMADO banida ou desconectada pela lógica principal
            if (isConfirmedBanned || realConnectionState === 'ban' || realConnectionState === 'disconnected') {
              this.logger.warn(
                `CONFIRMADO: Linha ${line.phone} está ${realConnectionState} na Evolution após verificação oficial. Realocando para operador ${operator.name}...`,
                'LineAvailability',
                {
                  ...
                  lineStatus: realConnectionState,
                },
              );
              ...
              this.logger.log(
                `Falso positivo evitado: Linha ${line.phone} voltou com status '${realConnectionState}' após verificação oficial (Verify). Mantendo linha.`,
                'LineAvailability'
              );
            }
          }
