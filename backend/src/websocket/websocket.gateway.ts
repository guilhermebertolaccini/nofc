  ...
  async handleSendMessage(
    ...
    try {
      ...
      if (
        ...
        if (segment && (segment as any).duploCpcEnabled === true) {
          const cpfToValidate = data.contactCpf || '';
          const contractToValidate = data.contactContract || '';
          ...
          // 1. Validar contrato na API do parceiro
          ...
          const canContact = await this.cpcService.checkAcionamento(
          ...
          while (templateAttempt < maxTemplateRetries) {
            ...
            const reallocationResult =
              await this.lineAssignmentService.reallocateLineForOperator(
                ...
                markAsBanned ? "message-sending" : undefined, // bannedReason
              );
            ...
            if (templateAttempt >= maxTemplateRetries) {
              ...
              for (const lineIdToBan of allFailedLines) {
                try {
                  ...
                  await this.linesService.handleBannedLine(lineIdToBan, "message-sending");
                } catch (banError) {
                  console.error(
            ...
            if (shouldReallocate) {
              ...
              const reallocationResult =
                await this.lineAssignmentService.reallocateLineForOperator(
                  ...
                  [], // excludeLineIds
                  undefined, // traceId
                  markLineAsBanned, // markAsBanned
                  markLineAsBanned ? "message-sending" : undefined, // bannedReason
                );
            ...
            if (isOurServer) {
              ...
              try {
                if (data.mediaUrl.startsWith("/media/")) {
                  ...
                  // URL completa: https://api.../media/arquivo.pdf
        ...
        try {
          if (data.messageType === "image" && data.mediaUrl) {
            ...
            try {
              if (data.mediaUrl.startsWith("/media/")) {
                ...
                const appUrl =
                  process.env.APP_URL ||
                  "https://api.taticamarketing.com.br";
          ...
          try {
            ...
            if (
              ...
              await this.linesService.handleBannedLine(newLineId, "message-sending");
              console.log(
