...
interface CrudTableProps<T extends { id: string }> {
  ...
  /** Campos adicionais para incluir na busca (além dos valores das colunas) */
...
export function CrudTable<T extends { id: string }>({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      ...
      // Search in additional fields (e.g. realNumber when column shows phone)
      const matchesSearchFields = searchFields?.some((key) => {
        const value = getValue(item, String(key));
        return String(value ?? '').toLowerCase().includes(query);
      });
      return matchesColumn || (matchesSearchFields ?? false);
    });
  }, [data, searchQuery, columns, searchFields, getValue]);
