...
const getSettings = () => {
  try {
    let raw = localStorage.getItem('tatica-settings');
    if (!raw && localStorage.getItem('vend-settings')) {
      raw = localStorage.getItem('vend-settings');
      if (raw) {
        localStorage.setItem('tatica-settings', raw);
        localStorage.removeItem('vend-settings');
      }
    }
    if (raw) {
