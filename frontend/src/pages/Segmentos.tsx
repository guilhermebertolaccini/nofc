...
export default function Segmentos() {
  ...
  const [formData, setFormData] = useState({ name: '', allowsFreeMessage: true, duploCpcEnabled: false, identifier: 'proprietario' as 'cliente' | 'proprietario' });
  ...
  const mapApiToLocal = (apiSegment: APISegment): Segment => ({
    ...
    duploCpcEnabled: (apiSegment as any).duploCpcEnabled ?? false,
  ...
  const handleAdd = () => {
    ...
    setFormData({ name: '', allowsFreeMessage: true, duploCpcEnabled: false, identifier: 'proprietario' });
    setIsFormOpen(true);
  };

  const handleEdit = (segment: Segment) => {
    setEditingSegment(segment);
    setFormData({ name: segment.name, allowsFreeMessage: segment.allowsFreeMessage, duploCpcEnabled: segment.duploCpcEnabled ?? false, identifier: segment.identifier || 'proprietario' });
    setIsFormOpen(true);
  };
    ...
    try {
      if (editingSegment) {
        const updated = await segmentsService.update(parseInt(editingSegment.id), {
          ...
          duploCpcEnabled: formData.duploCpcEnabled,
  ...
  const renderForm = () => (
    <div className="space-y-4 py-4">
      ...
      <div className="flex items-center space-x-2">
        <Checkbox
          id="duploCpcEnabled"
          checked={formData.duploCpcEnabled}
          onCheckedChange={(checked) => setFormData({ ...formData, duploCpcEnabled: checked === true })}
        />
        <Label htmlFor="duploCpcEnabled" className="text-sm font-normal cursor-pointer">
          Duplo CPC (validação via API externa)
        </Label>
      </div>
      <div className="text-xs text-muted-foreground">
        {formData.duploCpcEnabled
          ? "Operadores devem informar CPF (3 dígitos) e Contrato. Validação via API do parceiro antes do envio."
          : "Sem validação CPC externa para este segmento"}
      </div>
      <div className="space-y-2">
