...
export const setAuthToken = (token: string | null) => {
  authToken = token;
  if (token) {
    localStorage.setItem('tatica_token', token);
    localStorage.removeItem(LEGACY_TOKEN_KEY);
  } else {
    localStorage.removeItem('tatica_token');
    localStorage.removeItem(LEGACY_TOKEN_KEY);
  }
};
...
export const getAuthToken = (): string | null => {
  ...
  const next = localStorage.getItem('tatica_token');
  if (next) return next;
  const legacy = localStorage.getItem(LEGACY_TOKEN_KEY);
  if (legacy) {
    localStorage.setItem('tatica_token', legacy);
    localStorage.removeItem(LEGACY_TOKEN_KEY);
    return legacy;
  }
  return null;
...
const reportEndpoints: Record<string, string> = {
  ...
  'mensagens_por_usuario': 'mensagens-por-usuario',
  'linhas_banidas_por_usuario': 'linhas-banidas-por-usuario',
